...
function renderPreview() {
  ...
  // Check for missing RAW and Lens Correction files.
  let allMissing = [];
  let missingCount = 0;
  ...
  const lensComparison = FSAccess.compareLensToRaw();
  if (!lensComparison.matches) {
    const details = [
      ...lensComparison.missingFromLens.map(name => `${name}.* (missing from Lens Correction)`),
      ...lensComparison.unexpectedInLens.map(name => `${name}.* (does not match a RAW file)`),
      ...lensComparison.duplicates.map(item => `${item.basename}.* (duplicate logical filename)`)
    ];
    for (const detail of details) {
      allMissing.push(detail);
      missingCount++;
    }
  }
  ...
  if (missingCount > 0) {
    ...
    DOM.previewStatus.textContent = `⚠ ${missingCount} missing RAW/Lens file(s) — will be skipped`;
...
function renderPreviewTree(previews) {
  ...
  for (const p of previews) {
    ...
    for (const item of p.raw) {
      ...
      const cls = item.exists ? 'found' : 'missing';
      const label = item.exists ? escapeHtml(item.filename || item.basename + '.*') : `Missing: ${escapeHtml(item.basename)}.*`;
