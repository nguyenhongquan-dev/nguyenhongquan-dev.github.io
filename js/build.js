...
window.Builder = (() => {
  ...
  async function build(params) {
    ...
    for (const group of groups) {
      ...
      for (let i = 0; i < rawBasenames.length; i++) {
        ...
        if (!rawFile) {
          allErrors.push(`Missing RAW: ${base}.*`);
          log(`  ❌ Missing: ${base}.*`, 'error');
          continue;
        }
  ...
  function generatePreview(groups) {
    return groups.map(group => {
      ...
      // RAW and Lens Correction must be a one-to-one match.
      p.missingFiles = [];
      for (const item of p.raw) {
        if (!item.exists) p.missingFiles.push(`${item.basename}.* (RAW)`);
      }
      for (const item of p.lc) {
        if (!item.exists) p.missingFiles.push(`${item.basename}.* (Lens Correction)`);
      }
